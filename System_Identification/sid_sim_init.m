...
sim.param.PILOT_Y_EXPO = getParamVal(sid, 'PILOT_Y_EXPO'); % Acro yaw expo to allow faster rotation when stick at edges
sim.param.PILOT_Y_RATE = getParamVal(sid, 'PILOT_Y_RATE');
...
sim.signals.MOTB.ThrAvMx = single(sid.MOTB.ThrAvMx(iVec)); % Throttle average max
...
if isfield(sid, 'PSCD')
    ...
    if sim.param.rateCtrlVal || sim.param.attCtrlVal || sim.param.mdlVal || sim.param.altCtrlVal
        ...
        sim.init.PIDA.I = single(sim.signals.CTUN.ThrIn(1) - sim.signals.CTUN.ThrHov(1)) * 1000 ...
            - sim.param.PIDA.P * (sid.PIDA.Tar(1) - sid.PIDA.Act(1)) ...
            - sim.param.PIDA.FF * sid.PIDA.Tar(1); % Integrator init. according to AC_PosControl::init_z_controller()
